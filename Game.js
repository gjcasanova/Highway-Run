class Game {
  ...
  _getRandomX(start) {
    if (start) {
      return Math.floor(Math.random() * (200 + 1)); // 0- 200
    } else {
      return 200 + Math.floor(Math.random() * (200 + 1)); // 0 - 400
    }
  }
  ...
  _generateTrucks() {
    ...
    this.interval = setInterval(() => {
      const newTruck = new Truck(this._getRandomX(this.truckStart), this._getRandomY(), 100, 100, 'blue');
      this.truckStart = !this.truckStart;
  ...
  _stopGame() {
    ...
    document.getElementById('Game-Over').className = 'centeredgo';
    ...
    // document.getElementById("Game-Over").stile =
  }
